...
class ScheduleRepositoryImpl implements ScheduleRepositoryInterface
{
    ...
    public function getStudentSchedules(string $id, string $day): array
    {
        $sql = <<<SQL
            ...
            JOIN jadwal_mahasiswa ON (jadwal_mahasiswa.id_jadwal = jadwal_perkuliahan.id_jadwal)
    ...
    public function isScheduleConflict(ScheduleRequest $schedule): bool
    {
        ...
        $sql = <<<SQL
            SELECT id_jadwal, id_dosen, id_mata_kuliah, hari, waktu_mulai, waktu_selesai, ruangan
            FROM jadwal_perkuliahan
            WHERE (
                    id_dosen = :id_dosen
                    AND hari = :hari
                ...
                OR (
                    ...
                    AND (
                        ...
                        OR (
                            :waktu_selesai BETWEEN waktu_mulai AND waktu_selesai
                        )
                    )
                );
        SQL;
        ...
        if ($statement->rowCount() > 0) {
            ...
            while ($schedule_from_db = $statement->fetch(PDO::FETCH_ASSOC)) {
                if ($schedule_from_db["id_mata_kuliah"] != $learning_sub_id && $schedule_from_db["id_dosen"] == $lecturer_id && $schedule_from_db["hari"] == $day && $schedule_from_db["waktu_mulai"] == $starts_at && $schedule_from_db["waktu_selesai"] == $ends_at && $schedule_from_db["ruangan"] == $room) {
                } else {
